...
jojo 是一套设计系统，以纯文件的形式发布：一个 CSS 入口、一组设计令牌，以及
20 个 React 基础组件。CSS 层没有构建步骤，也没有运行时依赖。你可以链接一个
样式表、引用已解析的令牌，并按需从一个全局命名空间中取用这些基础组件。
...
通过读取已解析的语义令牌来为你自己的标记设置样式。这些别名会在浅色与深色
主题下自动解析到正确的值（深色是根元素上的 `.dark` 类）。下面是几个常用令牌：

- `--bg-app` —— 页面背景
- `--bg-card` —— 表面背景
- `--fg-1` —— 主文本；`--fg-2` —— 次文本
- `--accent-orange` —— 链接、激活状态和焦点状态
- `--rule-ink` —— 对比度更高的边框；`--hairline` —— 标准边框
...
每个基础组件都附带一个用于类型的 `.d.ts` 和一个描述其用途的 `.prompt.md`，
两者都与组件一起放在 [`components/`](../../components/) 中。组件变体会映射到共享
的令牌别名。
...
- [`index.html`](../../index.html) —— 概览 SPA：令牌、字体样张和组件示例。
- [`ui_kits/marketing/`](../../ui_kits/marketing/) —— 一个由基础组件组合而成的营销页面，并带有浅色/深色切换。
- [`ui_kits/console/`](../../ui_kits/console/) —— 一个用 jojo 构建的示例产品界面。

## 下一步

- 阅读[设计语言](./design-language.md)，了解项目约束。
- 在 [`components/`](../../components/) 中浏览基础组件和样张卡片，以及项目的 [`README.md`](../../README.md)。

---

© 2026 perelmangao，采用 MIT 许可证。
